...
class WebServer
{
	private:
		...
		friend int HttpGetHandler( struct soap *soap );
		friend int HttpPostHandler( struct soap *soap, const char *endpoint, const char *host, int port, const char *path, const char *action, size_t count );

		int port;
		char* ip;
		bool isListening;
		UPCBWebServiceSoapService* soap;
